...
The next preview pass uses app-rendered editor images at desktop and phone sizes. The phone frame has a screen inset so its rounded shell does not clip the app header. Correction now shows Detected family, Merge with and Not a rhyme with the actual family labels. Rhyme Lab includes its Mosaic map, sound lanes and combination builder, with scores measured from the engine. Share Studio includes its real format choices, layout panel, preview and Copy image / Save PNG actions. These excerpts keep the owner's existing words and family colors. `previews.css` contains their presentation rules; the README records capture and scoring conditions.
